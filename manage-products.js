document.addEventListener('DOMContentLoaded', () => {
    ...
    async function updateOrderBadge() {
        try {
            ...
            if (orderBadge) {
                previousOrderCount = data.count; // ตั้งค่าจำนวนเริ่มต้น
                if (data.count > 0) {
    ...
    const openModalForEdit = (productId) => {
        ...
        if (product.image) {
            imagePreview.src = product.image.startsWith('blob:') ? product.image : `https://softstep-backend.onrender.com/${product.image}`;
        ...
        cards.forEach(card => {
            ...
            deleteButton.addEventListener('click', async (event) => {
                ...
                if (confirm(`Are you sure you want to delete product ID: ${productId}?`)) {
                    try {
                        const response = await fetch(`https://softstep-backend.onrender.com/api/products/${productId}`, { method: 'DELETE' });
            ...
            toggle.addEventListener('change', async (event) => {
                ...
                try {
                    const response = await fetch(`https://softstep-backend.onrender.com/api/products/${productId}/status`, {
                        method: 'PATCH',
    ...
    productForm.addEventListener('submit', async (event) => {
        ...
        try {
            let response; let url = 'https://softstep-backend.onrender.com/api/products'; let method = 'POST';
            if (currentEditId) {
                url = `https://softstep-backend.onrender.com/api/products/${currentEditId}`; method = 'PUT';
            }
